			       ...
			       int pack_objects_out_fd)
{
	...
	/*
	 * Detect early exit via stdout hang-up; POLLHUP is reported even
	 * with events == 0.  The files are emitted after preparing packing
	 * data, before object enumeration or stdout output.
	 * Leave reaping to finish_pack_objects_cmd().
	 */
				...
				int pack_objects_out_fd)
{
	...
	/*
	 * Only the parent may hold the write end, or the aggregator
	 * would not see EOF when we exit.
	 */
	if (pipe(pp))
