...
--------
[synopsis]
git backfill [--min-batch-size=<n>] [--timeout=<seconds>]
	     [--[no-]sparse] [--[no-]include-edges]
	     [--[no-]progress] [<revision-range>]
...
`--timeout=<seconds>`::
	Stop the backfill after the specified number of seconds have
	elapsed. The timeout is checked after each batch finishes
	downloading, allowing the process to stop without leaving a
	partial packfile. A later `git backfill` invocation can continue
	downloading the remaining objects.
...
`--no-progress`::
	...
	progress reporting even if not attached to a terminal, or
	disables it when attached to a terminal.
