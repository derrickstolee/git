...
--improve-reused-deltas::
	When reusing an existing delta, also search for a better delta using
	the configured window and depth. The existing delta is retained unless
	the search finds a smaller delta, or one of the same size with a
	shallower depth. This option is incompatible with `--no-reuse-delta`.
...
`tree:0`, `object:type=<type>`, `sparse:<oid>`, and `treesparse:<oid>`.
These supported filter types can be combined with the
`combine:<spec>+<spec>` form. The `treesparse:<oid>` filter (see
linkgit:git-rev-list[1]) requires `--path-walk` and will cause an
error if `--path-walk` is not in effect.
