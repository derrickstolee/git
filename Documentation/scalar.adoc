...
scalar clone [--single-branch] [--branch <main-branch>] [--full-clone]
	...
	[--local-cache-path <path>] [--scope <scope>] <url> [<enlistment>]
scalar list
...
--scope <scope>::
	Set the `gvfs.prefetchScope` config value, which is sent as the
	`scope` parameter on `gvfs/prefetch` requests. This corresponds to a
	sparse-checkout scope for which the server has precomputed a
	restricted prefetch packfile set that excludes trees outside of
	that sparse-checkout cone.
