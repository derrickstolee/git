...
--[no-]progress::
	Show progress while creating packs. The default is to show progress
	for `--once` when standard error is connected to a terminal, and to
	suppress progress for `--loop`.
...
--max-objects=<n>::
	Skip packs estimated to contain more than `<n>` objects, leaving
	them untouched by step 2.  The object count is estimated cheaply
	from the size of the pack's `.idx` file rather than by opening it.
	The estimate assumes the version 2 index format, which is Git's
	default. Legacy version 1 indexes are smaller and may therefore
	admit modestly more objects than the requested limit.
